...
>(({ className, ...props }, ref) => (
  <ol
    ref={ref}
    className={cn(
      "flex flex-wrap items-center gap-1.5 break-words text-lg text-zinc-500 sm:gap-2.5 dark:text-zinc-400",
      className
    )}
    {...props}
  />
...
>(({ asChild, className, href, ...props }, ref) => {
  // Determina el componente a renderizar
  const Comp = asChild ? Slot : href ? Link : "span";

  if (href && !asChild) {
    // Renderiza Link si `href` está definido y `asChild` no está activo
    return (
      <Link
